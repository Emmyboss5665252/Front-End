<template>
  <CenterCard>
    <Form class="w-full" @submit.prevent="register">
      <FormGroup type="text" label="Title" v-model="form.title" :error="errorBag.title" />
...
function register() {
  ...
  if (form.title == "")
  errorBage.title = "Please enter your title"
  else 
  errorBage.title =""

  if (form.password != "" && form.email != "" && form.name != ""  && form.title !="") {
    errorBag.email = ""
    errorBag.password = ""
    errorBag.title = ""
    errorBag.name = ""
